...
int main(int argc, char* argv[])
{
    ...
    for(int loop = 0; loop < test_count; loop++)
    {
        ...
        std::vector<std::vector<COMPLEX_C>> vec_GPU(batch, std::vector<COMPLEX_C>(n * 2));  // A
        ...
        THROW_IF_CUDA_ERROR(cudaMalloc(&Forward_InOut_Datas, batch * n * 2 * sizeof(COMPLEX)));
...
./cmake-build-debug/     --parallel     --target fft_bench
./cmake-build-debug/fft_bench
*/
...
-B./cmake-build-debug
 cmake --build ./cmake-build-debug/ --target fft_bench
--parallel
