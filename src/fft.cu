            ...
            switch (cfg.n_power)
            {
                ...
                case 13:
                    ForwardCore<<<dim3(16, 1, batch_size), dim3(32, 8),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 0, 4, cfg.n_power,
                        cfg.zero_padding, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 16, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 4, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 14:
                    ForwardCore<<<dim3(32, 1, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 0, 5, cfg.n_power,
                        cfg.zero_padding, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 32, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 5, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 15:
                    ForwardCore<<<dim3(64, 1, batch_size), dim3(8, 32),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 0, 6, cfg.n_power,
                        cfg.zero_padding, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 64, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 6, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 16:
                    ForwardCore<<<dim3(128, 1, batch_size), dim3(4, 64),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 0, 7, cfg.n_power,
                        cfg.zero_padding, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 128, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 7, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 17:
                    ...
                    ForwardCore<<<dim3(16, 16, batch_size), dim3(32, 8),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 4, 4, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 256, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 8, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 18:
                    ...
                    ForwardCore<<<dim3(32, 16, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 4, 5, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 512, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 9, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 19:
                    ...
                    ForwardCore<<<dim3(32, 32, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 5, 5, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 1024, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 10, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 20:
                    ...
                    ForwardCore<<<dim3(64, 32, batch_size), dim3(8, 32),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 5, 6, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 2048, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 11, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 21:
                    ...
                    ForwardCore<<<dim3(64, 64, batch_size), dim3(8, 32),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 6, 6, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 4096, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 12, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 22:  //
                    ...
                    ForwardCore<<<dim3(128, 64, batch_size), dim3(4, 64),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 6, 7, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 8192, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 13, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                ...
                case 23:
                    ...
                    ForwardCore<<<dim3(128, 128, batch_size), dim3(4, 64),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 7, 7, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 16384, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 14, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 24:
                    ...
                    ForwardCore<<<dim3(128, 256, batch_size), dim3(4, 64),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 8, 7, cfg.n_power,
                        false, true);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    ForwardCore<<<dim3(1, 32768, batch_size), dim3(256, 1),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 15, 9, cfg.n_power,
                        false, false);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
            ...
            switch (cfg.n_power)
            {
                case 12:
                    ...
                    InverseCore<<<dim3(8, 1, batch_size), dim3(64, 4),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 2, 0, 3, cfg.n_power,
                ...
                case 13:
                    ...
                    InverseCore<<<dim3(16, 1, batch_size), dim3(32, 8),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 3, 0, 4, cfg.n_power,
                        cfg.mod_inverse, true, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 14:
                    ...
                    InverseCore<<<dim3(32, 1, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 4, 0, 5, cfg.n_power,
                        cfg.mod_inverse, true, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 15:
                    ...
                    InverseCore<<<dim3(64, 1, batch_size), dim3(8, 32),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 5, 0, 6, cfg.n_power,
                        cfg.mod_inverse, true, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 16:
                    ...
                    InverseCore<<<dim3(128, 1, batch_size), dim3(4, 64),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 6, 0, 7, cfg.n_power,
                        cfg.mod_inverse, true, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 17:
                    ...
                    InverseCore<<<dim3(16, 16, batch_size), dim3(32, 8),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 7, 4, 4, cfg.n_power,
                        cfg.mod_inverse, false, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                ...
                case 18:
                    ...
                    InverseCore<<<dim3(32, 16, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 8, 4, 5, cfg.n_power,
                        cfg.mod_inverse, false, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                ...
                case 19:
                    ...
                    InverseCore<<<dim3(32, 32, batch_size), dim3(16, 16),
                                  512 * sizeof(COMPLEX), cfg.stream>>>(
                        device_inout, root_of_unity_table, 9, 5, 5, cfg.n_power,
                        cfg.mod_inverse, false, false,
                        (batch_size << cfg.n_power));
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
