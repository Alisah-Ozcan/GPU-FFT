...
struct Complex
{
    ...
    __host__ __device__ __forceinline__ const double& real() const { return data.x; }  //
    __host__ __device__ __forceinline__ const double& imag() const { return data.y; }

    // Constructor to initialize the complex number
    __host__ __device__ __forceinline__ Complex(double r, double i) : data({r, i}) {}
    __host__ __device__ __forceinline__ Complex() : data({0.0, 0.0}) {}

    // Overload the addition operator (+) within the namespace
    __host__ __device__ __forceinline__ Complex operator+(const Complex& other) const
    {
...
struct Complex
{
    ...
    __host__ __device__ __forceinline__ const float& real() const { return data.x; }  //
    __host__ __device__ __forceinline__ const float& imag() const { return data.y; }
...
typedef std::complex<float> COMPLEX_C;
#else
#error "Please define either USE_FLOAT or USE_DOUBLE to choose the complex type."
#endif
