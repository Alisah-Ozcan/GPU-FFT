...
FFT::FFT(int size)
{
    ...
    x = x_;
    max_size = n * 2;
    root = 2.0 * M_PI / static_cast<float>(max_size);
...
void FFT::GenerateRootTable()
{
    ...
    for(int i = 0; i < n; i++)
    {
        COMPLEX_C element = std::exp(j * static_cast<float>(i) * root);
...
std::vector<COMPLEX_C> FFT::InverseReverseRootTable()
{
    ...
    for(int i = 0; i < n; i++)
    {
        inverse_reverse_root_table.push_back(inverse_root_tables[bitreverse(i, lg)]);
    }
