...
int main(int argc, char* argv[])
{
    ...
    vector<vector<unsigned long long>> A_poly(batch, vector<unsigned long long>(n * 2));
    vector<vector<unsigned long long>> B_poly(batch, vector<unsigned long long>(n * 2));
    ...
    std::vector<std::vector<COMPLEX_C>> A_vec(batch, std::vector<COMPLEX_C>(n * 2));
    std::vector<std::vector<COMPLEX_C>> B_vec(batch, std::vector<COMPLEX_C>(n * 2));
    ...
    {
        THROW_IF_CUDA_ERROR(cudaMemcpy(Forward_InOut_Datas + (n * 2 * j), vec_GPU[j].data(), n * 2 * sizeof(COMPLEX),
    ...
    THROW_IF_CUDA_ERROR(cudaMemcpy(test, Forward_InOut_Datas, batch * n * 2 * sizeof(COMPLEX), cudaMemcpyDeviceToHost));

    for(int j = 0; j < batch; j++)
    {
        vector<unsigned long long> test_school = schoolbook_poly_multiplication(A_poly[j], B_poly[j], q, n);
        for(int i = 0; i < n * 2; i++)
        {
