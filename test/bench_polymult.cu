    ...
    for (int loop = 0; loop < test_count; loop++)
    {
        ...
        THROW_IF_CUDA_ERROR(
            cudaMalloc(&Forward_InOut_Datas,
                       2 * batch * n * 2 *
                           sizeof(COMPLEX)));  // 2 --> A and B, batch --> batch
                                               // size, 2 --> zero pad
        ...
        GPU_NTT(Forward_InOut_Datas, Root_Table_Device, cfg_fft, batch * 2);

        GPU_NTT(Forward_InOut_Datas, Inverse_Root_Table_Device, cfg_ifft,
                batch);
